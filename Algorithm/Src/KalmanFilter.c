...
void Kalman_Filter_Init(KalmanFilter_t *kf, uint8_t xhatSize, uint8_t uSize, uint8_t zSize)
{
    ...
    kf->temp_matrix_data0 = (float *)user_malloc(sizeof_float * kf->xhatSize * kf->xhatSize);
    kf->temp_matrix_data1 = (float *)user_malloc(sizeof_float * kf->xhatSize * kf->xhatSize);
    kf->temp_vector_data0 = (float *)user_malloc(sizeof_float * kf->xhatSize);
    kf->temp_vector_data1 = (float *)user_malloc(sizeof_float * kf->xhatSize);
    Matrix_Init(&kf->S, kf->xhatSize, kf->xhatSize, (float *)kf->S_data);
    Matrix_Init(&kf->temp_matrix0, kf->xhatSize, kf->xhatSize, (float *)kf->temp_matrix_data0);
    Matrix_Init(&kf->temp_matrix1, kf->xhatSize, kf->xhatSize, (float *)kf->temp_matrix_data1);
    Matrix_Init(&kf->temp_vector0, kf->xhatSize, 1, (float *)kf->temp_vector_data0);
...
void Kalman_Filter_xhatMinusUpdate(KalmanFilter_t *kf)
{
    if (kf->SkipEq1 == 0)
    {
        if (kf->uSize > 0)
        {
            kf->temp_vector0.numRows = kf->xhatSize;
            kf->temp_vector0.numCols = 1;
            kf->MatStatus = Matrix_Multiply(&kf->F, &kf->xhat, &kf->temp_vector0);
            kf->temp_vector1.numRows = kf->xhatSize;
            kf->temp_vector1.numCols = 1;
            kf->MatStatus = Matrix_Multiply(&kf->B, &kf->u, &kf->temp_vector1);
            kf->MatStatus = Matrix_Add(&kf->temp_vector0, &kf->temp_vector1, &kf->xhatminus);
        }
        else
        {
...
void Kalman_Filter_PminusUpdate(KalmanFilter_t *kf)
{
    if (kf->SkipEq2 == 0)
    {
        kf->MatStatus = Matrix_Transpose(&kf->F, &kf->FT); // 计算转置
        kf->MatStatus = Matrix_Multiply(&kf->F, &kf->P, &kf->Pminus);
        kf->temp_matrix0.numRows = kf->Pminus.numRows;
        kf->temp_matrix0.numCols = kf->FT.numCols;
        kf->MatStatus = Matrix_Multiply(&kf->Pminus, &kf->FT, &kf->temp_matrix0); // temp_matrix0 = F * P(k-1) * FT
        kf->MatStatus = Matrix_Add(&kf->temp_matrix0, &kf->Q, &kf->Pminus);
    }
}

// K(k) = P'(k) × Hᵀ × (H × P'(k) × Hᵀ + R)^(-1)
void Kalman_Filter_SetK(KalmanFilter_t *kf)
{
    if (kf->SkipEq3 == 0)
    {
        kf->MatStatus = Matrix_Transpose(&kf->H, &kf->HT); // z|x => x|z
        kf->temp_matrix0.numRows = kf->H.numRows;
        kf->temp_matrix0.numCols = kf->Pminus.numCols;
        kf->MatStatus = Matrix_Multiply(&kf->H, &kf->Pminus, &kf->temp_matrix0); // temp_matrix0 = H * P'(k)
        kf->temp_matrix1.numRows = kf->temp_matrix0.numRows;
        kf->temp_matrix1.numCols = kf->HT.numCols;
        kf->MatStatus = Matrix_Multiply(&kf->temp_matrix0, &kf->HT, &kf->temp_matrix1); // temp_matrix1 = H * P'(k) * HT
        kf->S.numRows = kf->R.numRows;
        kf->S.numCols = kf->R.numCols;
        kf->MatStatus = Matrix_Add(&kf->temp_matrix1, &kf->R, &kf->S); // S = H * P'(k) * HT + R
        kf->MatStatus = Matrix_Inverse(&kf->S, &kf->temp_matrix1); // temp_matrix1 = inv(H * P'(k) * HT + R) 计算逆矩阵
        kf->temp_matrix0.numRows = kf->Pminus.numRows;
        kf->temp_matrix0.numCols = kf->HT.numCols;
        kf->MatStatus = Matrix_Multiply(&kf->Pminus, &kf->HT, &kf->temp_matrix0); // temp_matrix0 = P'(k) * HT
        kf->MatStatus = Matrix_Multiply(&kf->temp_matrix0, &kf->temp_matrix1, &kf->K); // K
    }
}
...
void Kalman_Filter_xhatUpdate(KalmanFilter_t *kf)
{
    if (kf->SkipEq4 == 0)
    {
        kf->temp_vector0.numRows = kf->H.numRows;
        kf->temp_vector0.numCols = 1;
        kf->MatStatus = Matrix_Multiply(&kf->H, &kf->xhatminus, &kf->temp_vector0); // temp_vector0 = H * xhat'(k)
        kf->temp_vector1.numRows = kf->z.numRows;
        kf->temp_vector1.numCols = 1;
        kf->MatStatus = Matrix_Subtract(&kf->z, &kf->temp_vector0, &kf->temp_vector1); // temp_vector1 = z(k) - H * xhat'(k)
        kf->temp_vector0.numRows = kf->K.numRows;
        kf->temp_vector0.numCols = 1;
        kf->MatStatus = Matrix_Multiply(&kf->K, &kf->temp_vector1, &kf->temp_vector0); // temp_vector0 = K(k) * (z(k) - H * xhat'(k))
        kf->MatStatus = Matrix_Add(&kf->xhatminus, &kf->temp_vector0, &kf->xhat);
    }
}
...
void Kalman_Filter_P_Update(KalmanFilter_t *kf)
{
    if (kf->SkipEq5 == 0)
    {
        kf->temp_matrix0.numRows = kf->K.numRows;
        kf->temp_matrix0.numCols = kf->H.numCols;
        kf->temp_matrix1.numRows = kf->temp_matrix0.numRows;
        kf->temp_matrix1.numCols = kf->Pminus.numCols;
        kf->MatStatus = Matrix_Multiply(&kf->K, &kf->H, &kf->temp_matrix0);// temp_matrix0 = K(k) * H
        kf->MatStatus = Matrix_Multiply(&kf->temp_matrix0, &kf->Pminus, &kf->temp_matrix1);// temp_matrix1 = K(k) * H * P'(k)
