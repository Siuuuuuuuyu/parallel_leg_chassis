                 ...
                 double rec_time_out, uint16_t device_index)
{
    ...
    DM8009->mode = mode;
    DM8009->hcan = hcan;
    DM8009->lp.p_min = -12.5;
    DM8009->lp.p_max = 12.5;
    DM8009->lp.v_min = -45.0;
    DM8009->lp.v_max = 45.0;
    DM8009->lp.kp_min = 0.0;
    DM8009->lp.kp_max = 500.0;
    DM8009->lp.kd_min = 0.0;
    DM8009->lp.kd_max = 5.0;
    DM8009->lp.t_min = -54.0;
    DM8009->lp.t_max = 54.0;
...
void DM8009_fbdata(DM8009_motor_t *DM8009, uint8_t *data)
{
    ...
    {
        pos_fb = ((((int16_t)data[0]) << 8) + data[1]) * (360.0 / 8191) * (PI / 180);                  //->degree->rad
        vel_fb = (double)((int16_t)((((int16_t)data[2]) << 8) + data[3])) * (2 * PI) / 60 / 100;         //放大100倍数 rpm->rad/s
        tor_fb = (double)((int16_t)((((int16_t)data[4]) << 8) + data[5])) * (1.0 / 1000.0) * DM8009->Ct; // mA->A->N*m
        temperature = data[6];
    }
...
void DM8009_cmd_upgrade(DM8009_motor_t *DM8009, double torque)
{
    ...
    if (DM8009->mode == TO4_MODE)
    {
        float I_cmd = torque / DM8009->Ct;
        if (I_cmd > DM8009_IMAX)
            I_cmd = DM8009_IMAX;
        if (I_cmd < -DM8009_IMAX)
            I_cmd = -DM8009_IMAX;
        int16_t temp_cmd = (I_cmd / DM8009_IMAX) * 16384.0;
        if (temp_cmd > 16384.0)
            temp_cmd = 16384.0;
        if (temp_cmd < -163840.)
            temp_cmd = -16384.0;
